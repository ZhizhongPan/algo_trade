...
class DrawDown(stratanalyzer.StrategyAnalyzer):
    ...
    def calculateEquity(self, strat):
        ...
        # ret = strat.broker.cash
        # for instrument, shares in strat.broker.positions.iteritems():
        #     _bar = strat.feed.get_last_bar(instrument)
        #     if shares > 0:
        #         ret += strat.broker.get_barLow(_bar) * shares
        #     elif shares < 0:
        #         ret += strat.broker.get_barHigh(_bar) * shares
        # return ret

    def beforeOnBars(self, strat, bars):
        equity = self.calculateEquity(strat)
        self.__currDrawDown.update(bars.date_time, equity, equity)
