...
class EventEngine:
    ...
    __on_timer：私有方法，计时器固定事件间隔触发后，向事件队列中存入计时器事件
    start: 公共方法，启动引擎
    stop：公共方法，停止引擎
    ...
    def __process(self, event):
        ...
        [handler(event) for handler in self.__handlers[event.type_] if event.type_ in self.__handlers]
